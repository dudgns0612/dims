...
public class DIMS_Server {
	...
	public void serverOpen()
	{
		try
		{
			...
			switch(result)
			{
			case DatabaseHandler.DRIVER_INIT_ERROR : if(PRINT_LOG)System.out.println("[Server] JDBC드라이버 설정이 잘못됐습니다.");
													 controlPanel.Log("E", "Fail to set up JDBC Driver.");
													 controlPanel.off();	
													 return;
			case DatabaseHandler.LOGIN_FAIL_ERROR : if(PRINT_LOG)System.out.println("[Server] 데이터베이스에 로그인하지 못했습니다. 아이디 또는 비밀번호를 확인하세요"); 
			...
			try
			{
				while(SERVER_RUN)
				{
					...
					else if(type.equals(NetworkProtocols.ENROLL_BOARD_REQUEST))
					{
						...
						try
						{
							rr = handler.excuteQuery("select 카테고리번호 from 게시글_카테고리목록 where 카테고리이름  = '"+request.get("카테고리").toString()+"';");
							System.out.println();
							System.out.println("select 카테고리번호 from 게시글_카테고리목록 where 카테고리이름  = '"+request.get("카테고리").toString()+"';");
							rr.next();
						}
						catch(SQLException e)
						{
						...
						int category = (int)rr.getInt("카테고리번호");
						System.out.println(rr.getInt("카테고리번호")+"ㅇㅇ");
					...
					else if(type.equals(NetworkProtocols.ADMIN_ADD_TAP_REQUEST))
					{
						...
						qry = "insert into 게시글_카테고리목록(카테고리이름) values('"+request.get("name")+"')";
					...
					else if(type.equals(NetworkProtocols.BOARD_LIST_REQUEST))
					{
						...
						ResultSet rr = handler.excuteQuery("select 카테고리번호 from 게시글_카테고리목록 where 카테고리이름  = '"+request.get("category").toString()+"';");
						rr.next();
						ResultSet rs = handler.excuteQuery("select G.게시글번호, S.이름, G.게시글제목, G.작성일자, G.게시글본문, M.카테고리이름 from 사용자 S, 게시글 G, 게시글_카테고리목록 M "
								                         + "where S.학번=G.작성자 and G.카테고리="+rr.getString("카테고리번호")+" and M.카테고리번호=G.카테고리;");
	                ...
	                else if(type.equals(NetworkProtocols.PLUS_MINUS_TAP_REQUEST))
	                {
	                   ...
	                   String qry = "select * from 사용자 where 사용자등급 = '학생';";
	                   String plus = null;
	                   String minus = null;
	                   int sum;
	                   ResultSet rs = handler.excuteQuery(qry);
					 ...
					 else if(type.equals(NetworkProtocols.MESSAGE_RICIEVER_ALL_DELETE_REQUEST))
                     {
		                ...
		                else if(type.equals(NetworkProtocols.MESSAGE_REPLY_REQUEST))
		                {
						 ...
						 else if(type.equals(NetworkProtocols.ADMIN_DELETE_CATEGORY_COUNT_REQUEST))
						 {
							 ...
							 ResultSet rsss = handler.excuteQuery("select 카테고리번호 from 게시글_카테고리목록 where 카테고리이름 = '"+request.get("category")+"';");
							 rsss.next();
							 String qry = "select count(*) from 게시글 where 카테고리="+rsss.getInt("카테고리번호")+";";
							 ResultSet rs = handler.excuteQuery(qry);
							 try
							 {
								 rs.next();
						 ...
						 else if(type.equals(NetworkProtocols.ADMIN_DELETE_FINAL_REQUEST))
						 {
							 ...
							 ResultSet rsss = handler.excuteQuery("select 카테고리번호 from 게시글_카테고리목록 where 카테고리이름 = '"+request.get("category")+"';");
							 rsss.next();
							 String qry = "delete from 게시글 where 카테고리 = "+rsss.getInt("카테고리번호")+";";
							 System.out.println();
							 handler.excuteUpdate(qry);
							 
							 qry = "delete from 게시글_카테고리목록 where 카테고리이름 = '"+request.get("category")+"';";
							 handler.excuteUpdate(qry);
							 ...
							 if(constraint)
							 {
								 new Thread(()->{
									 ...
									 try
									 {
										 ...
										 controlPanel.Log("C", "Start send email..."+mailData.get("받을사람"));
						 ...
						 else if(type.equals(NetworkProtocols.ROOM_NUMBER_MODIFY_REQUEST))
						 {
							 ...
							 handler.excuteUpdate("update 학생 set 방번호 = '"+request.get("reqRoomNum")+"' where 학번 = '"+request.get("reqId")+"';");
							 handler.excuteUpdate("update 학생 set 층 = '"+request.get("reqRoomNum").toString().charAt(0)+"' where 학번 = '"+request.get("reqId")+"';");
						 ...
						 else if(type.equals(NetworkProtocols.ADMIN_VIDEO_LIST_REQUEST))
						 {
							 ...
							 ResultSet rs = handler.excuteQuery("select * from 비디오목록");
							 ...
							 while(rs.next())
							 {
								 String[] keys = {"비디오번호","비디오이름","비디오크기"};
								 Object[] values = {rs.getInt("비디오번호"),rs.getString("비디오이름"),rs.getInt("비디오크기")};
						 ...
						 else if(type.equals(NetworkProtocols.ADMIN_VIDEO_DELETE_REQUEST))
						 {
							 ...
							 handler.excuteUpdate("delete from 비디오목록 where 비디오번호 = "+deleteIdx);
		                  ...
		                  //복붙
		                   else if(type.equals(NetworkProtocols.MAIN_SCHEDULE_REQUEST))
		                   {
		                      ...
		                      String qry = "select * from 일정";
		                      JSONArray jsonArray = new JSONArray();
		                      
		                      ResultSet rs = handler.excuteQuery(qry);
		                      String[] keys = {"일정번호","일정이름","일시","분류","일시"};
		                      while(rs.next())
		                      {
		                         Object[] values = {rs.getInt("일정번호"),
		                                        rs.getString("일정이름"), 
