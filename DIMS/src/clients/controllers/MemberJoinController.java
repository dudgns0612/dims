...
public class MemberJoinController implements Initializable  {
   ...
   private void onDuplicate()
   {
      if(!idField.getText().equals(""))
      {
         ...
         {
            CustomDialog.showConfirmDialog("학번 양식이 잘못 되었습니다.", sManager.getStage());
         }
      }
      else
      {
   ...
   private void onSuccess()
   {
      ...
      while(true)
      {
         if(!ID_CHECK)
         {
            CustomDialog.showConfirmDialog("학번 중복확인을 해주세요.", sManager.getStage());
            break;
         }
         ...
         json.put("학번", id);    json.put("비밀번호", password);   json.put("질문", selectqus);
         json.put("답변", qusanwer);   json.put("소속학과", classcheck);   json.put("이름", name);
            ...
            while(true)
            {
               ...
               if(type.equals(NetworkProtocols.ID_DUP_RESPOND_OK))
               {
                  Platform.runLater(new Runnable() {
                     @Override
                     public void run() {
                        CustomDialog.showConfirmDialog("생성이 가능한 학번 입니다.", sManager.getStage());
                        ID_CHECK = true;
                     }
                  });
               }
               else if(type.equals(NetworkProtocols.ID_DUP_RESPOND_DENY))
               {
                  Platform.runLater(new Runnable() {
                     @Override
                     public void run() {
                        CustomDialog.showConfirmDialog("이미 등록이 되어 있는 학번 입니다.", sManager.getStage());
                     }
                  });
               }
